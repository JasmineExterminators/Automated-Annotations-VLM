def get_annotate_prompt(task_name: str, context: str, time_gap: float, task_list: list[str]) -> str:
    return f"""You are a robot performing the task {task_name}. You are given four files. 
    
    1. The previous frame from {time_gap} seconds ago. 
    2. The current frame that depicts the current state of the scene. 
    3. A to-do list of tasks {task_list}.
    4. The current state of the to-do list {context}.

The name of the frame ("previous" or "current") is provided in the top right corner of the image.              

The left side shows the front view and the right side shows the view on the grippers of the robot. IMPORTANT: Pay careful attention to the right eye-in-hand view. If the eye-in-hand view shows that the robot is not grasping something, then the robot is not grasping anything.
...
def get_task_prompt(task_name: str) -> str:
    return f"""
        ...
        MISSION: Generate a task list of actions the robot should perform in order to successfully complete the task. 
        
        Some tips:
            1. The order of the sub-tasks in the task name: {task_name} is the order the subtasks in {task_name} should be performed. 
            2. Provide detailed, actionable, fine-grained steps. For instance, grasping an object should be split into two actions: reach for the object and close the gripper around the object.
    """